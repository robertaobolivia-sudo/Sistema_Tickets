...
export const PAGE_ACCESS_BY_PERFIL = {
    ...
    'dashboard-acompanhamento': ['ADMIN', 'SUPERVISOR', 'ANALISTA'],
    clientes: ['ADMIN'],
    ...
    'relatorios-evolucao': ['ADMIN', 'SUPERVISOR'],
...
export function applyNavPermissions(pageButtons) {
    ...
    const clientesGroup = document.getElementById('navClientesGroup');
    const clientesAllowed = canAccessPage('clientes');
    if (clientesGroup) {
        clientesGroup.classList.toggle('hidden', !clientesAllowed);
    }
    const dashboardGroup = document.getElementById('navDashboardGroup');
    const dashboardAllowed = canAccessPage('dashboard');
    if (dashboardGroup) {
        dashboardGroup.classList.toggle('hidden', !dashboardAllowed);
    }
    const relatoriosGroup = document.getElementById('navRelatoriosGroup');
    const relatoriosAllowed = canAccessPage('relatorios');
    if (relatoriosGroup) {
        relatoriosGroup.classList.toggle('hidden', !relatoriosAllowed);
    }
    ...
    pageButtons.forEach(button => {
        ...
        if (button.dataset.clientesMode) {
            const allowed = canAccessPage('clientes');
            button.classList.toggle('hidden', !allowed);
            button.disabled = !allowed;
            return;
        }
        if (button.dataset.dashboardSub) {
            const allowed = canAccessPage('dashboard');
            button.classList.toggle('hidden', !allowed);
            button.disabled = !allowed;
            return;
        }
        if (button.dataset.relatoriosSub) {
            const allowed = canAccessPage('relatorios');
            button.classList.toggle('hidden', !allowed);
            button.disabled = !allowed;
            return;
        }
        if (button.id === 'navClientesToggle') {
            button.classList.toggle('hidden', !clientesAllowed);
            button.disabled = !clientesAllowed;
            return;
        }
        ...
        if (button.id === 'navDashboardToggle') {
            button.classList.toggle('hidden', !dashboardAllowed);
            button.disabled = !dashboardAllowed;
            return;
        }
        if (button.id === 'navRelatoriosToggle') {
            button.classList.toggle('hidden', !relatoriosAllowed);
            button.disabled = !relatoriosAllowed;
            return;
        }
