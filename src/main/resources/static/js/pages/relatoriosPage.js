...
function formatConexaoCarteiraRelatorio(ticket) {
    const partes = [];
    if (ticket?.conexao) partes.push(ticket.conexao);
    if (ticket?.carteira) partes.push(ticket.carteira);
...
function renderTabelaRelatorio(tickets) {
    ...
    lista.forEach(ticket => {
        ...
        row.innerHTML = `
            ...
            <td>${formatConexaoCarteiraRelatorio(ticket)}</td>
