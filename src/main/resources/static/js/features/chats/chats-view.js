 ...
 */
import { formatPriority } from '@shared/ui/presentation.js';
import {
    formatSlaBadgeHtml,
...
export const CHATS_TAB_INDEVIDOS = 'indevidos';
...
const STATUS_INDEVIDOS = new Set(['INDEVIDO']);
...
export function filterTicketsForChatsTab(tickets, tab) {
    ...
    } else if (tab === CHATS_TAB_INDEVIDOS) {
        filtered = list.filter(t => STATUS_INDEVIDOS.has(normalizeStatus(t?.status)));
    } else {
...
export function sortTicketsForChatsList(tickets, tab) {
    const list = Array.isArray(tickets) ? [...tickets] : [];
    if (tab !== CHATS_TAB_HISTORICO && tab !== CHATS_TAB_INDEVIDOS) {
        return list;
    }
...
/** Aba Chats para abrir conversa a partir de status do ticket (ex.: histórico do Contato). */
export function resolveChatsTabForTicketStatus(status) {
    const s = normalizeStatus(status);
    if (STATUS_INDEVIDOS.has(s)) {
        return CHATS_TAB_INDEVIDOS;
    }
    if (STATUS_HISTORICO.has(s)) {
        return CHATS_TAB_HISTORICO;
    }
    if (STATUS_FILA.has(s)) {
        return CHATS_TAB_FILA;
    }
    if (STATUS_CHATS.has(s)) {
        return CHATS_TAB_CHATS;
    }
    return CHATS_TAB_HISTORICO;
}

export function isTicketIndevido(ticket) {
    return STATUS_INDEVIDOS.has(normalizeStatus(ticket?.status));
}

export function canOfferClassificarIndevido(ticket) {
    return Boolean(ticket?.numeroTicket) && isTicketStatusAtivo(ticket?.status);
}

/** Sprint 305 — botão visível somente se ticket INDEVIDO e classificado hoje (UTC-3 / SP). */
export function canOfferReverterIndevido(ticket) {
    if (!ticket?.numeroTicket || !isTicketIndevido(ticket)) {
        return false;
    }
    if (!ticket.classificadoOperacionalEm) {
        return false;
    }
    const hojeSpStr = new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString().slice(0, 10);
    const diaClassificacao = String(ticket.classificadoOperacionalEm).slice(0, 10);
    return diaClassificacao === hojeSpStr;
}
...
export function getChatsEmptyMessage(tab) {
    ...
    if (tab === CHATS_TAB_INDEVIDOS) {
        return 'Nenhum chamado classificado como indevido.';
    }
...
export const CHATS_COMPOSER_INDEVIDO_MSG =
    'Chamado classificado como indevido. Conversa somente leitura.';
...
/** Chats: ticket sem contatoId — etiquetas indisponíveis (somente ContatoEtiqueta). */
export const CHATS_ETIQUETAS_SOURCE_INDISPONIVEL = 'indisponivel';

export const CHATS_ETIQUETAS_SEM_CONTATO_MSG =
    'Ticket sem Contato vinculado; etiquetas não disponíveis.';
...
export function resolveChatsEtiquetasSource(ticket) {
    return hasChatsPanelContatoId(ticket)
        ...
        : CHATS_ETIQUETAS_SOURCE_INDISPONIVEL;
}

export function canManageChatsEtiquetas(ticket) {
    return resolveChatsEtiquetasSource(ticket) === CHATS_ETIQUETAS_SOURCE_CONTATO;
}
...
/** Fallback sem contatoId: telefone de atendimento ou do cliente. */
function getChatsPanelContatoRowsLegacy(ticket) {
    const candidatos = [];
    const tel = ticket.atendimentoTelefone || ticket.telefoneContato || ticket.telefone;
    if (!isChatsPanelValueEmpty(tel)) {
        candidatos.push({ label: 'WhatsApp', value: tel });
    }
    return filterChatsPanelRows(candidatos);
}
 ...
 */
/** Rótulo Principal/Adicional para origem do atendimento (Sprint 292). */
export function formatAtendimentoOrigemTipoLabel(tipo) {
    if (tipo === 'PRINCIPAL') {
        return 'Principal';
    }
    if (tipo === 'ADICIONAL') {
...
/** Texto exibido: número + (Principal|Adicional). */
/** Indicador de origem por mensagem (Sprint 294). */
export function formatInteracaoTelefoneOrigemLabel(interacao) {
    return formatAtendimentoOrigemTipoLabel(interacao?.telefoneOrigemTipo);
}

export function buildChatsMsgOrigemIndicatorHtml(interacao) {
    const label = formatInteracaoTelefoneOrigemLabel(interacao);
    if (!label) {
        return '';
    }
    const tel =
        interacao?.telefoneOrigem != null && String(interacao.telefoneOrigem).trim() !== ''
            ? String(interacao.telefoneOrigem).trim()
            : '';
    const title = tel ? `${tel} (${label})` : label;
    return `<span class="chats-msg-origem" title="${escapeChatsHtml(title)}">${escapeChatsHtml(label)}</span>`;
}

export function formatAtendimentoOrigemDisplay(telefone, tipo) {
    const tel = telefone != null && String(telefone).trim() !== '' ? String(telefone).trim() : '';
    const tipoLabel = formatAtendimentoOrigemTipoLabel(tipo);
    if (!tel && !tipoLabel) {
        return null;
    }
    if (tel && tipoLabel) {
...
export function getChatsPanelChamadoRows(ticket, options = {}) {
    ...
    const origemAtendimento = formatAtendimentoOrigemDisplay(
        ticket.atendimentoTelefone,
        ticket.atendimentoTelefoneTipo
    );
    const candidatos = [
...
function buildChatsHistoricoItemInnerHtml(item, formatDateTime) {
    ...
    const origem = formatAtendimentoOrigemDisplay(
        item?.atendimentoTelefone,
        item?.atendimentoTelefoneTipo
    );
    if (origem) {
        metaBits.push(`<span class="chats-historico-item-origem">${escapeChatsHtml(origem)}</span>`);
    }
...
export function countTicketsByTab(tickets) {
    ...
    return {
        ...
        [CHATS_TAB_INDEVIDOS]: filterTicketsForChatsTab(list, CHATS_TAB_INDEVIDOS).length
    };
}
...
/** Sprint F20: título do header quando Cliente não identificado no ticket. */
export const CHATS_HEADER_CLIENTE_TITLE_FALLBACK = 'Cliente não informado';

const CHATS_HEADER_ARTE_URL_PREFIX_CLIENTE = '/uploads/clientes/header-chats/';

function isChatsHeaderDisplayTextEmpty(value) {
...
/** Aceita apenas URL pública de arte do Cliente (`/uploads/clientes/header-chats/`). */
export function sanitizeChatsHeaderArtePublicUrl(url) {
    return sanitizeChatsHeaderArtePublicUrlWithPrefix(url, CHATS_HEADER_ARTE_URL_PREFIX_CLIENTE);
}
...
/** Valor CSS para `--chats-cliente-header-bg-image`. */
export function getChatsClienteHeaderArteCssValue(url) {
...
/**
 * Resolve arte do header (Sprint F20): Cliente no ticket, URL no ticket (prefixo cliente), cache por clienteId.
 * @param {Map<number, string>|null} [clienteArteById]
 */
export function resolveChatsHeaderArteUrl(ticket, clienteArteById = null) {
    ...
    if (clienteId != null && clienteArteById?.size) {
        return sanitizeChatsHeaderArtePublicUrl(clienteArteById.get(Number(clienteId)));
    }
    return null;
}

/** Aplica ou remove background WL no `#chatsClienteHeader`. */
export function applyChatsClienteHeaderArte(headerEl, ticket, clienteArteById = null) {
    if (!headerEl) {
        return;
    }
    const bgLayer = headerEl.querySelector('.chats-cliente-header-bg');
    const arteUrl = resolveChatsHeaderArteUrl(ticket, clienteArteById);
    const cssValue = getChatsClienteHeaderArteCssValue(arteUrl);
    if (cssValue) {
        headerEl.classList.add('chats-cliente-header--has-custom-bg');
        bgLayer?.classList.remove('chats-cliente-header--bg-fallback');
        headerEl.style.setProperty('--chats-cliente-header-bg-image', cssValue);
    } else {
        headerEl.classList.remove('chats-cliente-header--has-custom-bg');
        bgLayer?.classList.add('chats-cliente-header--bg-fallback');
        headerEl.style.removeProperty('--chats-cliente-header-bg-image');
    }
}

/** Título principal do header: Cliente contratante (empresa/razão ou nome). */
export function getChatsHeaderClienteTitle(ticket) {
    if (!ticket?.numeroTicket) {
        return 'Selecione uma conversa';
    }
    if (!isChatsHeaderDisplayTextEmpty(ticket?.empresa)) {
        return String(ticket.empresa).trim();
    }
    if (!isChatsHeaderDisplayTextEmpty(ticket?.cliente)) {
        return String(ticket.cliente).trim();
    }
    return CHATS_HEADER_CLIENTE_TITLE_FALLBACK;
}
...
export function getChatsHeaderCanalBrandHtml(ticket) {
    ...
    if (key === 'SISTEMA') {
        return `<span class="chats-cliente-canal-label chats-cliente-canal-label--sistema">${escapeChatsHtml(label)}</span>`;
    }
...
export function buildChatsListCardHtml(ticket, tab, options = {}) {
    ...
    const waitIsoAttr =
        temporal.liveCounter && temporal.waitIso
            ...
            : '';
    const telExtra =
        typeof options.buildTelefonesBadge === 'function'
            ? options.buildTelefonesBadge(options.telefonesAdicionaisCount)
            : '';
    return `
            <span class="chats-list-item-main">
                <span class="chats-list-item-name">${escapeChatsHtml(nome)}</span>
                <span class="chats-list-item-ticket">${escapeChatsHtml(displayValue(ticket?.numeroTicket))}${telExtra}</span>
            </span>
