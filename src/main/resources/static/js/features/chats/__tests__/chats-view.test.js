...
import {
    ...
    formatAtendimentoOrigemDisplay,
    formatAtendimentoOrigemTipoLabel,
    buildChatsMsgOrigemIndicatorHtml,
    formatInteracaoTelefoneOrigemLabel,
...
describe('chatsView', () => {
    ...
    it('filtra aba indevidos', () => {
        const lista = filterTicketsForChatsTab(
            [...tickets, { numeroTicket: 'T4', status: 'INDEVIDO' }],
            CHATS_TAB_INDEVIDOS
        );
        expect(lista).toHaveLength(1);
        expect(lista[0].numeroTicket).toBe('T4');
        expect(resolveChatsTabForTicketStatus('INDEVIDO')).toBe(CHATS_TAB_INDEVIDOS);
        expect(canOfferClassificarIndevido({ numeroTicket: 'T1', status: 'ABERTO' })).toBe(true);
        expect(canOfferClassificarIndevido({ numeroTicket: 'T4', status: 'INDEVIDO' })).toBe(false);
        expect(isTicketIndevido({ status: 'INDEVIDO' })).toBe(true);
    });
    ...
    it('indicador de origem na mensagem do cliente', () => {
        expect(formatInteracaoTelefoneOrigemLabel({ telefoneOrigemTipo: 'ADICIONAL' })).toBe('Adicional');
        const html = buildChatsMsgOrigemIndicatorHtml({
            telefoneOrigem: '5512942833853',
            telefoneOrigemTipo: 'ADICIONAL'
        });
        expect(html).toContain('chats-msg-origem');
        expect(html).toContain('Adicional');
        expect(buildChatsMsgOrigemIndicatorHtml({})).toBe('');
    });
    ...
    it('resolve aba Chats por status do ticket', () => {
        expect(resolveChatsTabForTicketStatus('ABERTO')).toBe(CHATS_TAB_FILA);
        expect(resolveChatsTabForTicketStatus('EM_ATENDIMENTO')).toBe(CHATS_TAB_CHATS);
        expect(resolveChatsTabForTicketStatus('RESOLVIDO')).toBe(CHATS_TAB_HISTORICO);
        expect(resolveChatsTabForTicketStatus('CANCELADO')).toBe(CHATS_TAB_HISTORICO);
    });
    ...
    it('painel direito: oculta campos vazios e monta seções', () => {
        ...
        const comOrigem = getChatsPanelChamadoRows(
            {
                ...full,
        ...
        expect(comOrigem.some(r => r.label === 'Origem do atendimento')).toBe(true);
        expect(comOrigem.find(r => r.label === 'Origem do atendimento')?.text).toContain('Adicional');
        expect(formatAtendimentoOrigemTipoLabel('PRINCIPAL')).toBe('Principal');
        expect(formatAtendimentoOrigemDisplay('5511', 'PRINCIPAL')).toBe('5511 (Principal)');
    ...
    it('painel: deduplica empresa/carteira/telefone e contato sem atendente interno', () => {
        ...
        const comContato = getChatsPanelContatoRows(fenix);
        expect(comContato.some(r => r.label === 'Nome')).toBe(true);
        expect(comContato.some(r => r.label === 'WhatsApp')).toBe(true);
    });
    ...
    it('Sprint F32: contexto de etiquetas por contato ou indisponivel', () => {
        ...
        expect(canManageChatsEtiquetas(comContato)).toBe(true);
        expect(getChatsEtiquetasContextKey(comContato)).toBe('contato:7');
        const semContato = { numeroTicket: 'TK-2' };
        expect(resolveChatsEtiquetasSource(semContato)).toBe(CHATS_ETIQUETAS_SOURCE_INDISPONIVEL);
        expect(canManageChatsEtiquetas(semContato)).toBe(false);
        expect(getChatsEtiquetasContextKey(semContato)).toBe('ticket:TK-2');
        expect(CHATS_ETIQUETAS_SEM_CONTATO_MSG).toContain('Contato');
    });

    it('Sprint F38: ticket sem contatoId usa telefone do cliente no painel Contato', () => {
        const legado = {
            numeroTicket: 'TK-1',
            cliente: 'Maria',
            telefone: '11999990000',
        ...
        expect(contato.some(r => r.label === 'WhatsApp' && r.value === '11999990000')).toBe(true);
    ...
    it('arte header F20: só Cliente; rejeita uploads/conexoes; sem arte → null', () => {
        ...
        expect(sanitizeChatsHeaderArtePublicUrl('/uploads/conexoes/header-chats/x.png')).toBeNull();
        expect(getChatsClienteHeaderArteCssValue(clienteUrl)).toContain('url(');
        const cache = new Map([[9, clienteUrl]]);
        expect(
            resolveChatsHeaderArteUrl({ numeroTicket: 'TK-2', clienteId: 9 }, cache)
        ).toBe(clienteUrl);
    });

    it('header F20: título por Cliente', () => {
        expect(
            getChatsHeaderClienteTitle({
                numeroTicket: 'TK-1',
                cliente: 'Cliente Oficial',
        ...
        expect(
            getChatsHeaderClienteTitle({ numeroTicket: 'TK-3', cliente: '—' })
        ).toBe(CHATS_HEADER_CLIENTE_TITLE_FALLBACK);
    ...
    it('card da lista: sem avatar, só nome, ticket e tempo', () => {
        ...
        expect(htmlFila).not.toMatch(/avatar|<img|placeholder/i);
