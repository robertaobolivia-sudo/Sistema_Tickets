...
import { setLoggedAnalyst, refreshLoggedAnalystFromServer } from '@shared/auth/auth.js';
import { buildAnalystPhotoSrc, getAnalystPhotoUrl } from '@shared/ui/analyst-avatar.js';
import { resolvePerfilAcessoCode, renderPerfilAcessoBadge } from '@shared/permissions/permissions.js';
...
const perfilFooter = document.getElementById('perfilFooter');
const perfilCancelarBtn = document.getElementById('perfilCancelarBtn');
const perfilSalvarBtn = document.getElementById('perfilSalvarBtn');

const perfilInputs = {
    nomeCompleto: document.getElementById('perfilNomeCompletoInput'),
    cpf: document.getElementById('perfilCpfInput'),
    dataNascimento: document.getElementById('perfilDataNascimentoInput'),
    cep: document.getElementById('perfilCepInput'),
    rua: document.getElementById('perfilRuaInput'),
    numero: document.getElementById('perfilNumeroInput'),
    bairro: document.getElementById('perfilBairroInput'),
    cidade: document.getElementById('perfilCidadeInput'),
    estado: document.getElementById('perfilEstadoInput'),
    uf: document.getElementById('perfilUfInput'),
    pais: document.getElementById('perfilPaisInput'),
    celular: document.getElementById('perfilCelularInput'),
    email: document.getElementById('perfilEmailInput'),
};

const groupFields = {
    identidade: ['nomeCompleto', 'cpf', 'dataNascimento'],
    endereco: ['cep', 'rua', 'numero', 'bairro', 'cidade', 'estado', 'uf', 'pais'],
...
function openProfilePhotoView() {
    ...
    const photoUrl = getAnalystPhotoUrl(getLoggedAnalyst());
    if (!photoUrl) {
        alert('Nenhuma foto cadastrada.');
        return;
    }
    if (perfilFotoVisualizacao) {
        perfilFotoVisualizacao.src = buildAnalystPhotoSrc(photoUrl, getLoggedAnalyst()?.id);
    }
...
function enterEditMode(group) {
    if (activeEditGroup && activeEditGroup !== group) exitEditMode();
    const analyst = getLoggedAnalyst() || {};
    (groupFields[group] || []).forEach(key => {
        const span = perfilFields[key];
        const input = perfilInputs[key];
        if (!span || !input) return;
        input.value = analyst[key] != null ? String(analyst[key]) : '';
        span.classList.add('hidden');
        input.classList.remove('hidden');
    });
    document.querySelectorAll('.perfil-edit-btn').forEach(b => b.classList.remove('active'));
    document.querySelector(`.perfil-edit-btn[data-target="${group}"]`)?.classList.add('active');
    perfilFooter?.classList.remove('hidden');
    const alertEl = document.getElementById('alertBoxPerfil');
    if (alertEl) { alertEl.textContent = ''; alertEl.classList.add('hidden'); }
    activeEditGroup = group;
    const firstKey = groupFields[group]?.[0];
    if (firstKey) perfilInputs[firstKey]?.focus();
}

function exitEditMode() {
    if (!activeEditGroup) return;
    (groupFields[activeEditGroup] || []).forEach(key => {
        const span = perfilFields[key];
        const input = perfilInputs[key];
        if (!span || !input) return;
    ...
    document.querySelector(`.perfil-edit-btn[data-target="${activeEditGroup}"]`)?.classList.remove('active');
    perfilFooter?.classList.add('hidden');
...
async function saveEditGroup() {
    ...
    try {
        const updated = await analistaService.update(analyst.id, payload);
        setLoggedAnalyst(updated);
        exitEditMode();
        renderPerfil();
        showPerfilToast('Alterações salvas com sucesso.');
    } catch (err) {
        showAlert(err.message || 'Erro ao salvar alterações.', alertBoxPerfil);
    }
}

function showPerfilToast(msg) {
    let toast = document.getElementById('perfilToast');
    if (!toast) {
...
export function initPerfilPage(deps = {}) {
    ...
    perfilCancelarBtn?.addEventListener('click', exitEditMode);
    perfilSalvarBtn?.addEventListener('click', saveEditGroup);
}