...
import { buildTicketBuscaParams } from '@shared/ui/query-params.js';
import { slicePageItems } from '@shared/ui/list-pagination.js';
...
function excludeIndevidoFromTicketsList(tickets) {
    if (!Array.isArray(tickets)) {
        return [];
    }
    return tickets.filter(t => String(t?.status ?? '').toUpperCase() !== 'INDEVIDO');
}

async function loadTicketTableDefault() {
    const seq = ++ticketTableLoadSeq;
    try {
        const tickets = excludeIndevidoFromTicketsList(await ticketService.listTickets('ALL'));
        if (seq !== ticketTableLoadSeq) {
            return;
        }
        cachedTicketsList = Array.isArray(tickets) ? tickets : [];
